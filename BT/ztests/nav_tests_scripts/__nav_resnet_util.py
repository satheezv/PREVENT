...
def capture_image(image_count):
    save_path = "Dataset_chemspeed\\combined_test_24_03_25"
...
def test_resnet18(image_path):
    ...
    model.load_state_dict(torch.load("resnet18_chemspeed_plate_detection_40_v2.pth", map_location=device))
...
def main():
    ...
    # if _calib:
    #     publish_ros_msg("CS_retractToDrive", _seq, 0, 0)  # Home message
    #     while not is_kuka_finished(ros_topic_reader,_seq, mode="LBR"):  # Loop until the robot finishes the task
    #         time.sleep(1)  # Check the status every second
    #     _seq += 1
    ...
    print("last seq id: ", _seq)
        ...
        #         time.sleep(1)  # Check the status every second
        #     _seq += 1
        #     publish_ros_msg("DrivePos", _seq, 2, 26)  # Home message
        #     while not is_kuka_finished(ros_topic_reader,_seq,mode="KMR"):  # Loop until the robot finishes the task
        #         time.sleep(1)  # Check the status every second
        #     _seq += 1
        #     publish_ros_msg("CS_CalibrationAndMoveCheck", _seq, 2, 27)  # Home message
        #     while not is_kuka_finished(ros_topic_reader,_seq, mode="LBR"):  # Loop until the robot finishes the task
