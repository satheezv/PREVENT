...
def create_manipulation_behavior_tree(task_node_id, Resnet_vision_model_path, vision_class_map = None, user_name="Satheesh"):
    ...
    robot_calibration_behavior = ExecuteArmTaskBehavior(node_id=task_node_id, task_name="Main Arm Task", task_type=2, name="robotManipulationBehavior", program_name="CalibrationAndMoveCheck")    
    ...
    task_or_retract_behavior = ExecuteCancelUnlockRetractBehavior(node_id=task_node_id, task_name="PickupRack/ArmRetract", task_type=0, name="retractBehavior", program_name="rackTransfer", stopUnlock_robot=True)  # Stop Unlock
    task_or_retract_status_behavior = IsKukaManipulationCompleted(name="retract Status Check", topic_name="/kuka2/task_status")

    move_gasSensor_ToCheck_behavior = ExecuteObjectHandlingBehavior(node_id=task_node_id, task_name="Pickup IoT sensor Module", task_type=0, name="Move Sensor to check Behavior", program_name="sensorToCheck")
    ...
    move_gasSensor_ToBase_behavior = ExecuteObjectHandlingBehavior(node_id=task_node_id, task_name="Return IoT sensor Module", task_type=0, name="Move Sensor to base Behavior", program_name="sensorToBase")
